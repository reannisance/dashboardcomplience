...
st.markdown("Pilih jenis pajak, upload file, dan pilih sheet untuk mulai analisis.")
...
if uploaded_file:
    ...
    normalized_cols = [col.upper().strip() for col in df_input.columns]
    col_mapping = dict(zip(normalized_cols, original_cols))
    ...
    if missing_cols:
        ...
        # Kembalikan ke title-case agar sidebar tidak KeyError
        df_output.columns = [col_mapping.get(col, col).title().strip() if col in col_mapping else col.title().strip() for col in df_output.columns]
